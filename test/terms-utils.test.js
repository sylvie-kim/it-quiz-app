...
test('CRUD의 작업 이름은 독립 용어 Read의 동의어로 취급하지 않는다', () => {
    ...
    assert.equal(merged.length, baseTerms.length + 1);
...
test('명시된 Rails 동의어는 Ruby on Rails와 같은 용어로 병합한다', () => {
    ...
    assert.equal(merged.length, baseTerms.length);
...
test('CRUD 전체 영문 확장명은 CRUD와 병합하지만 개별 작업 이름은 병합하지 않는다', () => {
    ...
    assert.equal(mergedFullName.length, baseTerms.length);
    assert.equal(
    ...
    for (const operation of ['Create', 'Read', 'Update', 'Delete']) {
        ...
        assert.equal(mergedOperation.length, baseTerms.length + 1, `${operation}는 독립 용어여야 한다`);
    }
});
